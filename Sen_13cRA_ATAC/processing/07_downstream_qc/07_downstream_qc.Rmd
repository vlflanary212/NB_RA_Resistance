...
tss_gr <- muRtools::getAnnotGrl.gencode("gencode.v21")[["gene"]]
...
tsse_scores <- data.frame(
  sample = character(), tss_enrichment = numeric(), stringsAsFactors = FALSE
)
...
for (i in seq_along(sample_list)) {
  ...
  tsse_scores <- rbind(
    tsse_scores, 
    data.frame(sample = sample_list[i], tss_enrichment = enrichment_score)
    )
