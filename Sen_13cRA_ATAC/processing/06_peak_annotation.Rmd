...
NarrowPeaksToGRanges <- function(peak_dir, sample_id) {
  peak_file <- file.path(
    peak_dir, sample_id, paste0(sample_id, "_peaks.narrowPeak")
  )
...
peaks <- paste0(seqnames(peaks), "_", start(peaks), "_", end(peaks))
```

```{r save peaks}
saveRDS(
  peak_list,
  here("Sen_13cRA_ATAC", "data", "peak_list.rds")
)
...
bam_files <- file.path(
  bam_dir, sample_list$sample, paste0(sample_list$sample, "_final.bam")
)
```

```{r create metadata df}
metadata <- sample_list |>
  separate(col = sample, sep = "_", into = c("sample", "replicate"))
